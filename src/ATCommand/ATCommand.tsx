...
export const atCommandClient = axios.create({
  baseURL: 'https://at-command.xiga.io'
})
atCommandClient.interceptors.response.use((res: AxiosResponse<any>) => {
  if (!res.data.success) {
...
export const ATCommand = () => {
  ...
  useEffect(() => {
    atCommandClient.get<void, ATCommandItem[]>('/at-command').then(res => {
  ...
  return (
    <div className='space-x-2 w-screen h-screen overflow-hidden'>
      <div className={'h-full bg-gray-300 p-4 w-70 float-left overflow-auto'}>
        <div className={'flex flex-row space-x-2 mb-4'}>
          <input className={'p-2 w-45'} placeholder={'Type to Filter'} value={filter} onChange={e => setFiler(e.target.value.trim().toUpperCase())} />
